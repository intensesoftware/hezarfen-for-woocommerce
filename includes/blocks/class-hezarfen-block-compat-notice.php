...
class Hezarfen_Block_Compat_Notice {
	...
	/**
	 * Tutorial video explaining how to switch the checkout page back to the
	 * classic shortcode.
	 */
	const VIDEO_URL = 'https://youtu.be/vFvAxlEIAKc';
	...
	public function maybe_render_notice() {
		...
		$video_url  = esc_url( self::VIDEO_URL );
		?>
		<div class="notice notice-warning">
			...
			<p class="description">
				...
				printf(
					...
					esc_html__( 'Or switch the checkout page back to the classic %1$s[woocommerce_checkout]%2$s shortcode, where Hezarfen\'s fields work without any WooCommerce upgrade.', 'hezarfen-for-woocommerce' ),
					'<code>',
					'</code>'
				);
				?>
			...
			<p>
				<a href="<?php echo $video_url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" target="_blank" rel="noopener noreferrer">
					<?php esc_html_e( 'Watch how to switch to the classic checkout', 'hezarfen-for-woocommerce' ); ?>
				</a>
				<?php
				if ( $edit_url ) {
					echo ' &middot; ';
					printf(
						...
						esc_html__( 'Edit the checkout page', 'hezarfen-for-woocommerce' )
					);
				}
				?>
			</p>
