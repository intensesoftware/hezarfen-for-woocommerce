...
class Encryption
{
    ...
    {
        if( ! self::is_encryption_key_generated() || ! defined('HEZARFEN_ENCRYPTION_KEY') )
            return false;
    ...
    public static function create_encryption_tester_text()
    {
        ...
        return update_option('_hezarfen_encryption_tester_text', self::encrypt("Istanbul"));
    }
    ...
    public static function encrypt($plaintext)
    {
        ...
        $ciphertext_raw = openssl_encrypt($plaintext, $cipher, self::getEncryptionKey(), $options=OPENSSL_RAW_DATA, $iv);
        $hmac = hash_hmac('sha256', $ciphertext_raw, self::getEncryptionKey(), $as_binary=true);
    ...
    public static function decrypt($ciphertext)
    {
        ...
        $original_plaintext = openssl_decrypt($ciphertext_raw, $cipher, self::getEncryptionKey(), $options=OPENSSL_RAW_DATA, $iv);
        $calcmac = hash_hmac('sha256', $ciphertext_raw, self::getEncryptionKey(), $as_binary=true);
