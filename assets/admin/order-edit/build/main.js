...
jQuery(document).ready($ => {
    ...
    $.post(ajaxurl, data, function (response) {
      ...
      if (response.success && response.data) {
        ...
        if (typeof dates === 'object' && !Array.isArray(dates)) {
          // Response is in format: { "SARIYER": ["2025-12-23", ...], "ZEKERİYAKÖY": [...] }
          let options = '<option value="">Randevu tarihi seçin</option>';
          ...
          // No dates available
          const message = response.data.message || 'Müsait tarih bulunamadı';
  ...
  function populateWarehouseDropdown(warehouses) {
    ...
    if (warehouses.length === 1) {
      $warehouseSelect.append($('<option></option>').val(warehouses[0].id).text(warehouses[0].label).attr('selected', 'selected'));
      ...
      $.each(warehouses, function (index, warehouse) {
        $warehouseSelect.append($('<option></option>').val(warehouse.id).text(warehouse.label).data('warehouse', warehouse));
      });
